...
try {
  ...
  if (workflow === 'compare') {
    ...
    const overviewRows = page.locator('[data-check="overview-table"] tbody tr')
    await overviewRows.first().waitFor()
    assert.equal(await overviewRows.count(), 1)
    assert.match(await overviewRows.first().innerText(), /木纤维替代构造/)
    assert.match(await overviewRows.first().innerText(), /-11\.54/)
    await button('01 构造编辑').click()
    ...
    const excluded = page.locator('[data-check="overview-excluded"]')
    await excluded.waitFor()
    assert.match(await excluded.innerText(), /木纤维替代构造/)
    assert.match(await excluded.innerText(), /构造面积不一致：基准为 100 平方米，该构造为 200 平方米。/)
    assert.equal(await overviewRows.count(), 0)
    await button('将替代构造统一为基准口径').click()
    await text('替代构造已按基准统一部位、面积和年限。').waitFor()
    assert.equal(await page.locator('[data-check="carbon-delta"]').innerText(), '-11.54')
    assert.equal(await page.locator('[data-check="overview-excluded"]').count(), 0)
    assert.match(await overviewRows.first().innerText(), /-11\.54/)
  }
