...
try {
  ...
  if (workflow === 'compose') {
    ...
    await button('米').click()
    assert.equal(await page.getByLabel('第 1 层厚度', { exact: true }).inputValue(), '0.1')
    assert.equal(await intensity.innerText(), '31.2')
    await page.getByText('构造总厚 0.1 米').waitFor()
    await page.getByLabel('第 1 层厚度', { exact: true }).fill('0')
    await text('第 1 层厚度需在 0.0001 至 2 米之间。').waitFor()
    await page.getByLabel('第 1 层厚度', { exact: true }).fill('0.15')
    assert.equal(await intensity.innerText(), '46.8')
    await button('毫米').click()
    assert.equal(await page.getByLabel('第 1 层厚度', { exact: true }).inputValue(), '150')
    await button('米').click()
    await button('毫米').click()
    assert.equal(await page.getByLabel('第 1 层厚度', { exact: true }).inputValue(), '150')
    assert.equal(await intensity.innerText(), '46.8')
  ...
  if (workflow === 'document') {
    await button('米').click()
    assert.equal(await page.getByLabel('第 1 层厚度', { exact: true }).inputValue(), '0.02')
    assert.equal(await intensity.innerText(), '90.1')
    ...
    await text('厚度（米）').waitFor()
    await page.getByText('厚度单位：米').waitFor()
    ...
    assert.ok(output.includes('厚度单位：米'))
    assert.ok(output.includes('厚 0.02 米'))
    assert.ok(output.includes('厚 0.0125 米'))
    await button('01 构造编辑').click()
