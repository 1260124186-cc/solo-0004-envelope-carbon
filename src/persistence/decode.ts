...
import { capacityLimits } from './types'
...
export function validateEnvelopeData(parsed: unknown): EnvelopeData {
  ...
  if (
    data.assemblies.length > capacityLimits.assemblies ||
    data.materials.length > capacityLimits.materials ||
    data.documents.length > capacityLimits.documents
  ) {
    throw new Error('数据条目超出当前版本容量。')
  }
...
export function decode(raw: string): EnvelopeData {
  try {
    return validateEnvelopeData(JSON.parse(raw))
  } catch (error) {
